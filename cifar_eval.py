...
def evaluate(X_test,y_test_lable):
	with tf.Graph().as_default() as g:
		...
		x_ = tf.placeholder(tf.float32, [None, cifar_train.n_input])	
		x = tf.reshape(x_, shape=[-1, 32, 32, 3])
		y = tf.placeholder(tf.float32, [None, cifar_train.n_classes])
	
		# Construct model
		pred = cifar_inference.inference(x, 1)     #dropout=1

		# Evaluate model
		correct_pred = tf.equal(tf.argmax(pred, 1), tf.argmax(y, 1))
		accuracy = tf.reduce_mean(tf.cast(correct_pred, tf.float32))
	
		test_batch_len =int( X_test.shape[0]/cifar_train.BATCH_SIZE)
		test_acc=[]
		...
		batchsize=cifar_train.BATCH_SIZE
